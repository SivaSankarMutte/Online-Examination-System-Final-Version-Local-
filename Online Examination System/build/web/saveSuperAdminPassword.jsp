<%-- 
    Document   : saveSuperAdminPassword
    Created on : 23 Feb, 2022, 4:29:23 PM
    Author     : SIVASANKAR
--%>
...
<html>
    <head>
        ...
        <title>Save SuperAdmin Password</title>
    </head>
    <body>
        ...
        %>
            ...
            <sql:update dataSource="${db}" var="add">  
                update superadmin set password=? where email like ?
        ...
        <c:choose>
            <c:when test="${add>=1}">
                ...
                <c:redirect url="superAdminPasswordResetSuccess.jsp"/>
            </c:when>
            
            <c:otherwise>
                <c:redirect url="superAdminPasswordResetFailed.jsp"/>
            </c:otherwise>
        </c:choose>
     <% }
    else{ 
